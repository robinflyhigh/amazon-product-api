...
const { writeFile, readFileSync, existsSync, appendFileSync } = require('fs');
...
class AmazonScraper {
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            case 'products':
                return 's';
            case 'reviews':
                return `product-reviews/${this.asin}/ref=cm_cr_arp_d_viewopt_srt?formatType=${CONST.reviewFilter.formatType[this.reviewFilter.formatType]
    ...
    async buildRequest(page) {
        ...
        try {
            ...
            if (this.scrapeType === 'asin') {
                let hash = md5(options.uri);
                let file = `cache/${hash}.html`
                ...
                */
                ...
                if (existsSync(file) && false) {
                    ...
                    //await fromCallback((cb) => writeFile(file, response.body, cb));
    ...
    extractProductInfromation($) {
        ...
        try {
            for (let id of this.geo.product_information.id) {
                ...
                console.log();
                console.log(id);
                console.log(informationSection.html());
                console.log();
                console.log();
                */
                    ...
                    for (let item of arr) {
                        ...
                        if (len === 1) {
                            try {
                                ...
                                } else if (id === '#productDetails_detailBullets_sections1') {
                            ...
                            try {
                                ...
                                if (id === '#productOverview_feature_div div tbody') {

                                    key = $(item).find('span').first().text();
                                    value = $(item).find('span').last().text();
                                } else {
                                    key = $(item).find('span.a-text-bold').first().text().replace(/[^\w\s]/gi, '');
                                    value = $(item).find('span').last().text();
                                }
                        ...
                        if (fieldToExtract) {
                            ...
                            if (fieldToExtract && value) {
                                ...
                                product_information[fieldToExtract.key] = value.trim();
                            }
                        } else {

                            if (key !== "") {
                                let saveKey = key.toLowerCase().replace(/ /g, '_');
                                product_information[saveKey] = value.trim();
                            }

                            /*
                            if (key !== "") {
                                let saveKey = key.toLowerCase().replace(/ /g, '_');
                                appendFileSync('keys.txt', "'" + key + "': { key :'" + saveKey + "'}," + "\n");
                                appendFileSync('info.txt', saveKey + ": ''," + "\n");
                            }
                            */
    ...
    grapAsinDetails(body) {
        ...
        output.status = $("title").text().replace('\\n', '').replace('\\n', '');
        ...
        if (!output.current_price) {
            try {
                if ($(`.apexPriceToPay`).length == 1) {
                    output.price.current_price = this.geo.price_format($(`.apexPriceToPay span`).first().text());
                } else {
                    output.price.current_price = this.geo.price_format($($(`.priceToPay`)[0].children[0]).text());
                }
            } catch (err) {
                // continue regardless of error
            }
        ...
        let price_discounted = $('.basisPrice span span').first();

        output.price.discounted = price_discounted ? true : false;
        output.price.before_price = output.price.discounted
            ? this.geo.price_format(price_discounted.text())
            ...
            moreBuyingOptions.children.forEach((item, index) => {
                try {
                    if (item.attribs.class && item.attribs.class === 'a-box mbc-offer-row pa_mbc_on_amazon_offer') {
                        ...
                        const price = $(`#mbc-price-${position}`).text();
                        ...
                        const url = `${this.mainHost}${$(`#mbc-popover-soldBy-${position}`).next()[0].attribs.href}`;

                        let urlParts = rUrl.parse(url, true).query.seller;
